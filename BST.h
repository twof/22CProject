...
template <class bstdata>
BST<bstdata>::BST() : size(0), root(NULL) {} // Default Constructor

template <class bstdata>
BST<bstdata>::BST(const BST &toCopy) { // Copy Constructor
	root = copyHelper(toCopy.root);
}

template <class bstdata>
BST<bstdata>::~BST() { // Destructor
	destructHelper(root);
	root = NULL;
}

template <class bstdata>
int BST<bstdata>::minimum() { // Minimum
	int min = root->rank;
	Nodeptr temp = root;
...
template <class bstdata>
int BST<bstdata>::maximum() { // Maximum
	int max = root->rank;
	Nodeptr temp = root;
...
template <class bstdata>
void BST<bstdata>::add(string country, int rank, int gold, int silver, int bronze) {
	if (root == NULL) {
...
template <class bstdata>
void BST<bstdata>::remove(int rankVal) {
	if (root != NULL) {
		if (root->rank == rankVal) {
...
template <class bstdata>
bool BST<bstdata>::search(int value) {
	if (isEmpty()) {
